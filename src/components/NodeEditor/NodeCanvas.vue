<template>
    <div class="node-editor-canvas" ref="canvasRef" @mousedown="onMouseDown" @mousemove="onMouseMove"
        @mouseup="onMouseUp" @wheel.prevent="onWheel" @contextmenu.prevent="onContextMenu">
        ...
        <svg class="connections-layer">
            <g :style="transformStyle">
                <ConnectionLine v-for="conn in connections" :key="conn.id"
                    ...
                    :value="getConnectionValue(conn.id)" @click.stop="onConnectionClick(conn.id)" />
                <!-- Dragging Connection Line -->
                <ConnectionLine v-if="connectionState && connectionState.isConnecting" :x1="dragStart.x"
                    :y1="dragStart.y" :x2="dragEnd.x" :y2="dragEnd.y" :value="null" />
            </g>
        </svg>
...
function onMouseDown(e: MouseEvent) {
    ...
    const isBackground = e.target === canvasRef.value || (e.target as HTMLElement).classList.contains('transform-layer');
...
function onMouseUp() {
    ...
    // If we released mouse and we were connecting, but not on a port (handled by connect-end), cancel
    if (connectionState.value?.isConnecting) {
        endConnection();
    }
}
...
function onConnectStart({ nodeId, type, port }: any) {
    // ONLY allow starting from OUTPUT
    if (type === 'input') return;

    const { x, y } = getPortPosition(nodeId, port, type);
    // Start connection
    startConnection(nodeId, port, x, y);
}
...
function onConnectEnd({ nodeId, type, port }: any) {
    ...
    if (state && state.isConnecting) {
        ...
        // Determine type of the source port
        const sourceNode = nodes.value.find(n => n.id === state.sourceNodeId);
        if (!sourceNode) return;

        // We know source is Output because we enforced it in onConnectStart
        // Must connect to an input
        if (type !== 'input') return;
...
function getPortPosition(nodeId: string, portName: string, type: 'input' | 'output') {
...
function createNodeAtPos(type: string, pos: { x: number, y: number }) {
    ...
    if (type === 'math') {
        ...
        node.label = 'Color';
        node.data = { inputFormat: 'rgb', outputFormat: 'rgb', r: 255, g: 0, b: 0, alpha: 1 };
        node.inputs = { input: null, r: null, g: null, b: null, alpha: null };
        node.outputs = { output: null, rgb: null, rgba: null, hex: null, hsv: null, r: null, g: null, b: null, h: null, s: null, v: null, alpha: null };
        ...
        node.data = { rows: 2, cols: 2, horizontalSpacing: 0.1, verticalSpacing: 0.1, subplotTypes: ['xy', 'xy', 'xy', 'xy'] };
        node.inputs = {};
