<template>
  <BaseNode :node="node" :selected="selected" class="styling-node" @connect-start="$emit('connect-start', $event)"
    ...
    <div class="styling-node-content">
          ...
          <div v-if="activeComp === 'palette'" class="comp-group">
            ...
            <div v-if="isDiscreteTrace" class="comp-subgroup">
              ...
              <div v-else-if="discreteLabels.length > 12" class="fallback-note">
                Many categories detected ({{ discreteLabels.length }}). Using
                JSON fallback.
              </div>
      ...
      <div v-else class="empty-state">Select a component to start styling</div>
    </div>
  </BaseNode>
</template>
...
const filteredComponents = computed(() => {
  ...
  const all = [
    {
      id: "marker",
      label: "Marker",
      icon: "i-mdi-dots-horizontal-circle",
      isFound: info?.hasMarker,
      supported: [
        "scatter",
        "bar",
        "pie",
    ...
    },
    {
      ...
      isFound: info?.hasFill,
      supported: ["scatter", "bar", "box", "violin", "histogram"],
    },
    {
      id: "palette",
      label: "Palette",
      icon: "i-mdi-palette-outline",
      isFound: info?.hasPalette,
      supported: [
        "heatmap",
        "pie",
        "sunburst",
  ...
  return all.filter(
    (c) =>
      c.supported === "*" || (trace.type && c.supported.includes(trace.type))
  );
});
...
const traceInfo = computed(() => {
  ...
  return {
    ...
    hasMarker:
      !!trace.marker ||
      ["scatter", "bar", "scatter3d", "scattergeo", "scattermapbox"].includes(
        trace.type
      ),
    ...
    hasPalette:
      !!trace.colorscale ||
      ["heatmap", "pie", "sunburst", "surface"].includes(trace.type),
  };
});
