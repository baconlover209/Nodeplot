...
const isSizeConnected = computed(() =>
  nodeEditorState.connections.some(
    (c) => c.targetNodeId === props.node.id && c.targetPort === "size"
  )
);

const isStepConnected = computed(() =>
  nodeEditorState.connections.some(
    (c) => c.targetNodeId === props.node.id && c.targetPort === "step"
  )
);

const isStartConnected = computed(() =>
  nodeEditorState.connections.some(
    (c) => c.targetNodeId === props.node.id && c.targetPort === "start"
  )
);
