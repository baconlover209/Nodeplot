                ...
                <div v-if="activeTab === 'general'" class="config-group">
                    ...
                    <div class="control-row">
                        <div class="input-wrap">
                            ...
                            <input type="text" v-model="config.title_text" @input="updateData"
                    ...
                    <div class="control-row">
                        <div class="input-wrap">
                            ...
                            <input type="text" v-model="config.font_family" @input="updateData"
                ...
                <div v-if="activeTab === 'grid'" class="config-group">
                    ...
                    <template v-if="config.use_grid">
                        <div class="control-row">
                            <div class="input-wrap">
                                <label>Rows</label>
                                <input type="number" v-model.number="config.grid_rows" min="1" max="10"
                                    @input="updateData">
                            </div>
                            <div class="input-wrap">
                                <label>Cols</label>
                                <input type="number" v-model.number="config.grid_cols" min="1" max="10"
                                    @input="updateData">
                            </div>
                        </div>
                        <div class="control-row">
                    ...
                    <div class="subplot-list">
                        <div v-for="(sub, idx) in config.subplots" :key="idx" class="subplot-item">
                            <div class="item-header">
                                ...
                                <input type="text" v-model="sub.id" placeholder="ID (e.g. 2)" style="width: 50px"
                                    @input="updateData">
                                <button class="remove-btn" @click="removeSubplot(idx)">×</button>
                            </div>
                        ...
                        <button class="add-btn" @click="addSubplot">+ Add Mixed/Inset Subplot</button>
                    </div>
                </div>
                ...
                <div v-if="activeTab === 'axes'" class="config-group">
                    ...
                    <div class="axis-list">
                        <div v-for="(axis, idx) in config.axes" :key="idx" class="axis-item">
                            <div class="item-header">
                                <div class="axis-type-badge" :class="axis.type">{{ axis.type.toUpperCase() }}{{ axis.id
                                    || '1' }}</div>
                            ...
                            <div class="control-grid-2">
                                ...
                                <div class="input-wrap">
                                    ...
                                    <select v-model="axis.side" @change="updateData">
                                        ...
                                        <option value="right" v-if="axis.type === 'y'">Right</option>
                                        <option value="bottom" v-if="axis.type === 'x'">Bottom</option>
                            ...
                            <div class="control-row" v-if="axis.id">
                                <div class="input-wrap">
                                    ...
                                    <input type="text" v-model="axis.overlaying" placeholder="e.g. y"
                                        @input="updateData">
                                </div>
                                <div class="input-wrap">
                                    <label>Anchor</label>
                            ...
                            <div v-if="axis.showDomain" class="domain-box">
                                <div class="domain-row">
                                    <label>Domain:</label>
                                    <input type="number" v-model.number="axis.domain_start" step="0.1"
                                        @input="updateData">
                                    <span>-</span>
                                    <input type="number" v-model.number="axis.domain_end" step="0.1"
                                        @input="updateData">
                                </div>
                    ...
                    <div class="trace-mapping-list" v-else>
                        <div v-for="(trace, idx) in inputTraces" :key="idx" class="mapping-item">
                            <div class="mapping-trace-info">
                                ...
                                <span class="trace-name">{{ trace.name || trace.type || 'Unnamed Trace' }}</span>
                            </div>

                            <div class="mapping-controls">
                                <template v-if="['scatter', 'bar', 'box', 'violin', 'histogram'].includes(trace.type)">
                                    <div class="input-wrap">
                                        <label>X Axis</label>
                                        <select v-model="getTraceMapping(idx).xaxis" @change="updateData">
                                            <option value="">Default (X1)</option>
                                            <option v-for="id in axisIds.x" :key="id" :value="id">X{{ id || '1' }}
                                            </option>
                                        </select>
                                    </div>
                                    <div class="input-wrap">
                                        ...
                                        <select v-model="getTraceMapping(idx).yaxis" @change="updateData">
                                            ...
                                            <option v-for="id in axisIds.y" :key="id" :value="id">Y{{ id || '1' }}
                                            </option>
                                        </select>
                                    </div>
                                </template>
                                ...
                                <template
                                    v-else-if="trace.type === 'scatter3d' || trace.type === 'surface' || trace.type === 'mesh3d'">
                                    <div class="input-wrap">
                                        <label>3D Scene</label>
                                        <select v-model="getTraceMapping(idx).scene" @change="updateData">
                                            <option value="">Default (Scene1)</option>
                                            <option v-for="id in subplotIds('scene')" :key="id" :value="id">Scene{{ id
                                                || '1' }}</option>
                                        </select>
                                    </div>
                                </template>
                                ...
                                <template v-else-if="trace.type.includes('mapbox')">
                                    <div class="input-wrap">
                                        ...
                                        <select v-model="getTraceMapping(idx).mapbox" @change="updateData">
                                            ...
                                            <option v-for="id in subplotIds('mapbox')" :key="id" :value="id">Mapbox{{ id
                                                || '1' }}</option>
                                        </select>
                                    </div>
                                </template>
                                ...
                                <template v-else-if="trace.type.includes('geo')">
                                    <div class="input-wrap">
                                        ...
                                        <select v-model="getTraceMapping(idx).geo" @change="updateData">
                                            ...
                                            <option v-for="id in subplotIds('geo')" :key="id" :value="id">Geo{{ id ||
                                                '1'
                                                }}</option>
                                        </select>
                                    </div>
                    ...
                    <div class="help-accordion">
                        <div v-for="section in helpSections" :key="section.id" class="help-section"
                            ...
                            <div class="help-header"
                                @click="expandedHelp = (expandedHelp === section.id ? '' : section.id)">
                                <span>{{ section.title }}</span>
                                <div :class="expandedHelp === section.id ? 'i-mdi-chevron-up' : 'i-mdi-chevron-down'">
                                </div>
                            </div>
...
const helpSections = [
    {
        ...
        content: 'Configure global chart properties like <b>Titles</b>, <b>Fonts</b>, and <b>Background Colors</b>. Margin offsets (T/B/L/R) control the padding between the plot and the paper edge.'
    },
    {
        id: 'grid',
        title: 'Subplot Grid',
        content: 'Create a regular grid of subplots using rows and columns. Use <b>Independent</b> pattern for separate data per subplot, or <b>Coupled</b> to share axes. <b>Specific Subplots</b> allows adding 3D, Polar, or Map regions.'
    },
    {
        id: 'axes',
        title: 'Multi-Axis Layout',
        content: 'Define secondary X and Y axes for mixed scales or dual-axis charts. Use <b>Domain</b> to set where an axis exists on the chart (0.0 to 1.0). <b>Position</b> allows shifting an axis to the right/top of its range.'
    },
    {
        id: 'traces',
        title: 'Trace Routing',
        content: 'Assign specific input series to your defined axes or subplots. This is key for creating <b>Insets</b> or <b>Side-by-Side</b> comparisons from a single "traces" input.'
    },
    {
        id: 'legend',
        title: 'Legend Positioning',
        content: 'Toggle the legend and set its orientation (V/H). Use <b>X/Y Position</b> to move it anywhere in or out of the plot area (e.g., X=1.05 puts it just outside the right edge).'
    },
    {
        id: 'insets',
        title: 'How to create Insets',
        content: '1. Add a new X and Y axis in the <b>Axes</b> tab.<br>2. Set their <b>Domains</b> to a small range (e.g., 0.6 to 0.9).<br>3. In the <b>Traces</b> tab, map the target series to these new axes (#2, #3, etc).'
    }
];
...
watch(() => props.node.data.layoutConfig, (newVal) => {
    if (newVal) {
