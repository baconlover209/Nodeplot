<template>
  <div ref="containerRef" class="node-container" :class="{ selected: selected }" :style="style"
    ...
    <div class="node-body" :style="{ minHeight: minBodyHeight + 'px' }">
      <!-- Ports are absolute positioned on the sides -->
      <div class="inputs">
...
import { startNodeDrag, registerSocketPosition, getNodeColor, layoutVersion } from './nodeEditorState';
  ...
  Object.keys(props.node.inputs).forEach(portKey => {
    ...
    if (socketEl) {
      ...
      if (handleEl) {
        ...
        const centerX = props.node.position.x + socketPos.x + handleEl.offsetWidth / 2;
        const centerY = props.node.position.y + socketPos.y + handleEl.offsetHeight / 2;

        registerSocketPosition(props.node.id, portKey, 'input', centerX, centerY);
      }
    }
  });
  ...
  Object.keys(props.node.outputs).forEach(portKey => {
    ...
    if (socketEl) {
      ...
      if (handleEl) {
        ...
        registerSocketPosition(props.node.id, portKey, 'output', centerX, centerY);
      }
    }
  });
}
