<template>
  <div class="control-tile" :class="{ editing: isEditing }">
    <div class="tile-header">
      <span class="tile-label" v-if="!isRenaming" @dblclick="startRenaming">{{ tile.label }}</span>
      <input 
        v-else 
        ref="renameInput"
        v-model="tempLabel" 
      ...
      <div v-if="isEditing" class="tile-actions">
        <!-- Layout Switcher -->
        <select v-model="tile.layout" class="layout-select" title="Change Layout">
            <option value="vertical">Vertical</option>
            <option value="horizontal">Horizontal</option>
            <option value="grid2x2">Grid 2x2</option>
        </select>
        <button class="action-btn delete-btn" @click="$emit('remove-tile', tile.id)" title="Remove Tile">×</button>
      </div>
    </div>
    ...
    >
      ...
      >
        <div v-if="isEditing" class="control-overlay">
            ...
            <button class="action-btn remove-item-btn" @click="$emit('remove-item', tile.id, item.id)" title="Remove Control">×</button>
        </div>
        
        <!-- Slot for rendering the actual control -->
...
const emit = defineEmits(['remove-tile', 'remove-item', 'update-tile', 'drop-item']);
...
function onDrop(event: DragEvent) {
    // Stop propagation so the parent (grid/tile container) doesn't receive this drop event
...
function onDragStart(event: DragEvent, index: number) {
    ...
    // Stop propagation so the parent (grid/tile container) doesn't capture this as a "tile drag"
    event.stopPropagation();
...
.control-wrapper {
    ...
    min-width: 0; /* Important for flex/grid items to shrink properly */
}

/* In horizontal mode, allow items to grow/shrink but have a basis */
